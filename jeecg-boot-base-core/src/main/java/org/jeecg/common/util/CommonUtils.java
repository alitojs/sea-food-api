...
public class CommonUtils {
    ...
    public static String getFileNameByUrl(String fileUrl) throws IOException {
        ...
        if(path.contains(SymbolConstant.SINGLE_SLASH)){
            return path.substring(path.lastIndexOf(SymbolConstant.SINGLE_SLASH) +1);
        }else {
            throw new RuntimeException("网络地址异常，无法获取到文件名称");
        }
    }
